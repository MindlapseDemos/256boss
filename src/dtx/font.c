...
#else
	printf("ignoring call to dtx_open_font: not compiled with freetype support!\n");
#endif
...
struct dtx_font *dtx_open_font_mem(void *ptr, int memsz, int fontsz)
{
	...
	if(FT_Open_Face(ft, &args, 0, (FT_Face*)&fnt->face) != 0) {
		printf("failed to open font from memory\n");
		free(fnt);
		return 0;
	}
...
#else
	printf("ignoring call to dtx_open_font_mem: not compiled with freetype support!\n");
#endif
...
void dtx_prepare(struct dtx_font *fnt, int sz)
{
	if(!dtx_get_font_glyphmap_range(fnt, sz, 0, 256)) {
		printf("%s: failed (sz: %d, range: 0-255 [ascii])\n", __func__, sz);
	}
}
...
void dtx_prepare_range(struct dtx_font *fnt, int sz, int cstart, int cend)
{
	if(!dtx_get_font_glyphmap_range(fnt, sz, cstart, cend)) {
		printf("%s: failed (sz: %d, range: %d-%d)\n", __func__, sz, cstart, cend);
	}
}
...
int dtx_calc_font_distfield(struct dtx_font *fnt, int scale_numer, int scale_denom)
{
	...
	while(gm) {
		if(dtx_calc_glyphmap_distfield(gm) == -1) {
			printf("%s failed to create distfield glyphmap\n", __func__);
			return -1;
		}

		if(dtx_resize_glyphmap(gm, scale_numer, scale_denom, DTX_LINEAR) == -1) {
			printf("%s: failed to resize glyhphmap during distfield conversion\n", __func__);
		}
		gm->tex_valid = 0;
		gm = gm->next;
	}
...
int dtx_calc_glyphmap_distfield(struct dtx_glyphmap *gmap)
{
	...
	if(!(new_pixels = malloc(num_pixels))) {
		printf("%s: failed to allocate %dx%d pixel buffer\n", __func__, gmap->xsz, gmap->ysz);
		return -1;
	}
	dptr = new_pixels;
...
int dtx_resize_glyphmap(struct dtx_glyphmap *gmap, int snum, int sdenom, int filter)
{
	...
	if((count_bits(snum) | count_bits(sdenom)) != 1) {
		printf("%s: invalid scale fraction %d/%d (not power of 2)\n", __func__, snum, sdenom);
		return -1;
	}
	...
	if(snum != 1 && sdenom != 1) {
		printf("%s: invalid scale fraction %d/%d (neither is 1)\n", __func__, snum, sdenom);
		return -1;
	}
...
static struct dtx_glyphmap *load_glyphmap(struct io *io)
{
	...
	while(hdr_lines < 3) {
		...
		if(line[0] == '#') {
			...
			if((ptr = strstr(line + 1, "size:"))) {
				gmap->ptsize = atoi(ptr + 5);

			} else if((ptr = strstr(line + 1, "advance:"))) {
				gmap->line_advance = atoi(ptr + 8);

			} else if(parse_glyphline(line + 1, &tmpglyph) != -1) {
				...
				*g = tmpglyph;
				...
				printf("%s: invalid glyph info line\n", __func__);
				goto err;
			}
			...
			case 0:
				if(line[0] != 'P' || !(line[1] == '6' || line[1] == '5')) {
					printf("%s: invalid file format (magic)\n", __func__);
					goto err;
				}
				greyscale = line[1] == '5';
				break;
			...
			case 2:
				{
					...
					if(endp == line) {
						printf("%s: invalid file format (maxval)\n", __func__);
						goto err;
					}
				}
				break;
	...
	if(gmap->ptsize == -1 || gmap->line_advance == FLT_MIN) {
		printf("%s: invalid glyphmap, insufficient information in ppm comments\n", __func__);
		goto err;
	}
	...
	for(i=0; i<num_pixels; i++) {
		...
		if(c == -1) {
			printf("unexpected end of file while reading pixels\n");
			goto err;
		}
...
static int parse_glyphline(const char *str, struct glyph *g)
{
	...
	g->code = strtol(str, &endp, 0);
	if(endp == str) return -1;

	str = skip_space(endp);
	if(*str++ != ':') return -1;

	g->width = strtod(str, &endp);
	if(endp == str || *(str = endp) != 'x') return -1;
	g->height = strtod(str + 1, &endp);
	if(endp == str + 1 || *(str = endp) != '+') return -1;
	g->x = strtod(str + 1, &endp);
	if(endp == str + 1 || *(str = endp) != '+') return -1;
	g->y = strtod(str + 1, &endp);
	if(endp == str + 1) return -1;

	str = skip_space(endp);
	if(str[0] != 'o' || str[1] != ':') return -1;
	str += 2;

	g->orig_x = strtod(str, &endp);
	if(endp == str || *(str = endp) != ',') return -1;
	g->orig_y = strtod(str + 1, &endp);
	if(endp == str + 1) return -1;

	str = skip_space(endp);
	if(strncmp(str, "adv:", 4) != 0) return -1;
	str += 4;
	g->advance = strtod(str, &endp);
	if(endp == str) return -1;
